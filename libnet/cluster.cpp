...
Cluster::key_t Cluster::get_key(const unit_t &unit) {
    return unit.host();
}


void Cluster::insert(map_t &map, const unit_t &unit) {
    map.insert(std::make_pair(get_key(unit), unit));
}
...
Cluster::map_t Cluster::list_to_map(const std::initializer_list<Cluster::unit_t> &m) {
    ...
    for (auto i = m.begin(); i < m.end(); ++i) {
        result.insert(std::make_pair((*i).host(), *i));
    }
    return result;
}
